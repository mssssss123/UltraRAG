...
In UR-v2, we adopt the MCP (Model Context Protocol) architecture to standardize and encapsulate these functional modules into a unified implementation—**Server**.
...
In UR-v2, all Servers are instantiated via the `UltraRAG_MCP_Server` base class. Example:
...
Here, we define the parameter `name` with the default value `"UltraRAG v2"`.
...
During the **build** phase, UR-v2 automatically reads each Server’s `parameter.yaml` file to detect and register the parameters required by its Tool functions.  
...
- **Automatic Context Inference:** If a Tool’s input parameter is not defined in `parameter.yaml`, UR-v2 will attempt to infer its value from the runtime context (i.e., from the output of upstream Tools). Therefore, only explicitly define parameters in `parameter.yaml` when they cannot be automatically inferred.
