...
### Deploying the Retriever Server

When evaluating multiple benchmarks or model variants on the same corpus, re-initializing the retriever server each time leads to repeated loading of large corpora and index files — a slow and inefficient process.

To address this, UR-v2 provides a persistent Retriever Server deployment script, allowing the retriever to stay alive on CPU or GPU, eliminating repeated loading and significantly accelerating experimentation.

**Step 1: Configure Parameters**

Prepare a configuration file similar to the standard retriever server setup:
...
**Step 2: Deploy in Background**

It is recommended to use Screen so the retriever server can run persistently and logs can be monitored at any time.
...
Start the retriever server:

```shell script/deploy_retriever_server.py
...
Once launched, the server will remain in memory and no longer needs to reload the corpus or index.

**Step 3: Online Retrieval**

When performing retrieval, you no longer need to initialize a retriever inside the pipeline.
Simply specify the deployed server’s URL:
...
### Saving Retrieved Documents

By default, UR-v2 stores runtime results in files like `output/memory_xxx.json`.
In many cases, however, you may want to write the retrieved documents back into the original dataset — for example, to facilitate downstream processing or to continue running additional pipelines.

To support this workflow, UR-v2 provides a utility script that merges retrieval results into the original dataset and outputs an updated file:
...
After execution, the script automatically loads the dataset, aligns retrieval results, and generates a new dataset containing the retrieved document fields.