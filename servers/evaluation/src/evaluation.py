...
## evisrag evaulation (unusual, only for this pipeline)

# Sufficient Accuracy Score
def sufficient_acc(gt: List[str], pred: str, is_suff: bool) -> float:
    if not is_suff:
        gt = ["insufficient to answer", "i cannot answer"]
...
# Sufficient F1 Score
def sufficient_f1(gt: List[str], pred: str, is_suff: bool) -> float:
    def calc_f1(gt_str: str, pred_str: str) -> float:
        pred_norm = normalize_text(pred_str)
        gt_norm = normalize_text(gt_str)
...
def evisrag_compute_metrics(
    ...
    METRICS_REGISTRY: Dict[str, Callable[[List[str], str], float]] = {
        ...
        "rouge-2": rouge2_score,
        "rouge-l": rougel_score,
        "sufficient_acc": sufficient_acc,
        "sufficient_f1": sufficient_f1,
    }
    if not metrics:
        metrics = list(METRICS_REGISTRY.keys())
    metrics = [m.lower() for m in metrics]
    ...
    for gt, pred, is_suff in zip(gt_list, pred_list, issuff_ls):
        for metric in metrics:
            if metric in METRICS_REGISTRY:
                if "sufficient" in metric:
                    score = METRICS_REGISTRY[metric](gt, pred, is_suff)
                else:
                    score = METRICS_REGISTRY[metric](gt, pred)
                results[metric].append(score)
            else:
...
@app.tool(output="pred_ls,gt_ls,issuff_ls,metrics,save_path->eval_res")
def evisrag_evaluate(
    pred_ls: List[str],
    gt_ls: List[List[str]],
    issuff_ls: List[bool],
    metrics: List[str] | None,
    ...
    results = evisrag_compute_metrics(gt_ls, pred_ls, issuff_ls, metrics)
