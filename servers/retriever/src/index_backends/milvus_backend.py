...
class MilvusIndexBackend(BaseIndexBackend):
    def __init__(
        ...
        **_: Any,
        ...
        self.index_params_cfg = dict(self.config.get("index_params", {}))
        self.search_params = dict(self.config.get("search_params", {}))
    ...
    def build_index(
        ...
        **kwargs: Any
        ...
        if not passed_contents:
            raise ValueError("[milvus] 'contents' is required for build_index in Demo mode.")
        ...
        try:
            ...
            self.logger.warning(f"[milvus] Flush warning (indexing might be delayed): {e}")
        try:
