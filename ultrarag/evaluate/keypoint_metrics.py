...
class KEYPOINT_METRICS:
    ...
    def __call__(self, doc, language="zh") -> float:
        ...
        ground_truth = doc['ground_truth']['references']
        predict_answer = doc['prediction']['references']
        ...
        for kp in key_points:
            for retry in range(MAX_RETRIES):
                try:
                    ...
                    warnings.warn(f"Error processing key point {kp}: {str(e)}")
                    time.sleep(3)
        
        # 计算满足率
        recall_ratio = self._evaluator_recall(ground_truth, predict_answer)
        ...
        doc["result"] = {"is_recall": recall_ratio, "completeness": completeness, "hallucination": hallu_ratio, "irrelevance": irrelevance, "responses": responses_text}
        return doc
    ...
    def simhash_compare(self, text1, text2, threshold = 5):
        hash1, hash2 = Simhash(text1).value, Simhash(text2).value
        hamming_distance = hash1.distance(hash2)
        return hamming_distance <= threshold


    def _evaluator_recall(self, ground_truth, predict, method="equal"):
        if 'equal' == method:
            predict_hash = set([hash(item) for item in predict])
            ground_truth_hash = set([hash(item) for item in ground_truth])
            return predict_hash & ground_truth_hash
        elif 'similar' == method:
            for text1 in ground_truth:
                for text2 in predict:
                    if self.simhash_compare(text1, text2):
                        return True
            return False
...
if __name__ == "__main__":
    ...
    args.add_argument("-src_file", required=True, help="The input test file needs to follow the template format.")
    args.add_argument("-dst_file", required=True, help="Output the evaluation results.")
    args = vars(args.parse_args())

    with jsonlines.open(args['src_file'], "r") as fr:
        test_data = list(fr)
    
    evaluator = KEYPOINT_METRICS(True)

    with jsonlines.open(args["dst_file"], "w") as fw:
