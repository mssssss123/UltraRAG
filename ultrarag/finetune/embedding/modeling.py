...
from .base import AbsEmbedderModel, EmbedderOutput
...
class EncoderOnlyEmbedderM3Model(AbsEmbedderModel):
    ...
    def __init__(
        ...
        if not self.unified_finetuning:
            ...
            self.sparse_linear = base_model['sparse_linear'].float()
    ...
    def _sparse_embedding(self, hidden_state, input_ids, return_embedding: bool = True):
        ...
        """
        max_hidden_norm = torch.max(torch.norm(hidden_state,dim=-1),dim = -1).values
        self.sparse_linear = self.sparse_linear.float()
        token_weights = torch.relu(self.sparse_linear(hidden_state.float()) / max_hidden_norm.unsqueeze(-1).unsqueeze(-1))
    ...
    def encode(self, features):
        ...
        if self.unified_finetuning:
            ...
            return dense_vecs.contiguous(), None
    ...
    def forward(
        ...
        """
        q_dense_vecs, q_sparse_vecs = self.encode(queries)  # (batch_size, dim)
        p_dense_vecs, p_sparse_vecs = self.encode(passages) # (batch_size * group_size, dim)
        ...
        if self.training:
            ...
            if self.unified_finetuning:
                ...
                if self.use_self_distill and self.step > self.self_distill_start_step:
                    ...
                    loss += dense_self_distill_loss + 0.1 * sparse_self_distill_loss
                    loss = loss / 2
            self.step += 1
        else:
            loss = None
