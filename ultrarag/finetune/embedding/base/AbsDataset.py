...
class AbsEmbedderTrainDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        query = data['query']
        if self.args.query_instruction_for_retrieval is not None and len(self.args.query_instruction_for_retrieval) > 0:
...
class AbsEmbedderSameDatasetCollator(DataCollatorWithPadding):
    ...
    def __call__(self, features):
        ...
        # warnings.warn("here we add a query instruction for minicpm")
        queries_inputs = self.tokenizer(
            queries,
            truncation=True,
            max_length=self.query_max_len,
