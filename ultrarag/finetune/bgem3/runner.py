...
class EncoderOnlyEmbedderM3Runner(AbsEmbedderRunner):
    ...
    def load_tokenizer_and_model(self) -> Tuple[PreTrainedTokenizer, AbsEmbedderModel]:
        ...
        if 'XLMRobertaModel' in config.architectures and not self.training_args.sentence_pooling_method:
            self.training_args.sentence_pooling_method = 'cls'
        elif 'MiniCPMModel' in config.architectures and self.training_args.sentence_pooling_method:
            self.training_args.sentence_pooling_method = 'mean'
        else:
            warnings.warn('The model is not supported, please check the model architecture')
