...
from ultrarag.workflow.ultraragflow import KBAlignFlow
...
def display_configurations():
    """
    Display KBAlign configurations in a Streamlit expander.
    Initializes necessary session states and handles model/collection setup.
    """
    # Initialize session states if not exists
    if "kbalign_inst" not in st.session_state:
        st.session_state.kbalign_inst = ""

    if "collection" not in st.session_state:
        st.session_state.collection = []

    if "kbalign_history" not in st.session_state:
        st.session_state.kbalign_history = []
        st.session_state.kbalign_messages = []

    if 'chat_config' not in st.session_state:
        st.session_state.chat_config = {}

    # Display configuration expander
    with st.expander("UltraRAG-KBAlign"):
        if 'selected_options' not in st.session_state:
            st.session_state['selected_options'] = []
        if 'current_kb_config_id' not in st.session_state:
            st.session_state['current_kb_config_id'] = None
            
        # Initialize KBAlign if knowledge base is loaded
        kb_df = st.session_state.get('kb_df')
        if (isinstance(kb_df, pd.DataFrame)) and not kb_df.empty:
            try:
                st.session_state.kbalign_inst = KBAlignFlow.from_modules(
...
async def listen(query, container):
    ...
    # Get response from KBAlign instance
    inst = st.session_state.kbalign_inst
    response = await inst.aquery(
        query=query, 
        messages=st.session_state.kbalign_messages, 
    ...
    async for chunk in response:
        ...
        if isinstance(chunk, dict):
            if chunk["state"].startswith("recall"): process_info = get_debug_fold(title=chunk["state"], context=chunk["value"])
            if chunk["state"].startswith("first_answer"): process_info += get_debug_fold(title=chunk["state"], context=chunk["value"])
            if chunk['state'].startswith("query_expansion_recall"): process_info += get_debug_fold(title=chunk["state"], context=chunk["value"])
...
def display():
    ...
    if st.session_state.get("kbalign_inst",""):
        chat_component()
