...
import inspect
from ultrarag.workflow.ultraragflow import AgentRAGFlow
...
def display_configurations():
    """
    Display Agent RAG configurations in a Streamlit expander.
    ...
    if "agentrag_flow_inst" not in st.session_state:
        st.session_state.agentrag_flow_inst = ""

    if "collection" not in st.session_state:
        st.session_state.collection = []

    if "agent_rag_history" not in st.session_state:
        st.session_state.agent_rag_history = []
        st.session_state.agent_rag_messages = []

    # Display configuration expander
    with st.expander("Agent RAG"):
        if 'selected_options' not in st.session_state:
            st.session_state['selected_options'] = []
        if 'current_kb_config_id' not in st.session_state:
            st.session_state['current_kb_config_id'] = None
            
        # Initialize AgentRAGFlow if knowledge base is loaded
        kb_df = st.session_state.get('kb_df')
        if (isinstance(kb_df, pd.DataFrame)) and not kb_df.empty:
            try:
                st.session_state.agentrag_flow_inst = AgentRAGFlow.from_modules(
...
async def listen(query, container):
    ...
    rag_link = st.session_state.agentrag_flow_inst
    response = await rag_link.aquery(query)
...
def display():
    ...
    if st.session_state.get("agentrag_flow_inst",""):
        chat_component()
