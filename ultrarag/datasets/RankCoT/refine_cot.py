...
class COTRefineGenerator:
    ...
    def create_dataset(self, input_data):
        ...
        """
        return COTRefineDataset(input_data, self.tokenizer, self.config)
    ...
    def run(self):
        ...
        sampling_params_dict = self.config.get('refine_sampling_params', {})
     

        all_save_list = self.generate(dataloader, sampling_params_dict)
 
        self.save_results(all_save_list)
        print(f"COT Refine data completed, results have been saved in real-time to {self.output_path}")


class COTRefineDataset(Dataset):
...
def main():
    ...
    parser.add_argument('--input_path', type=str, default='/data/groups/QY_LLM_Other/meisen/dataset/rankcot/result_data/querypassage_to_CoT.jsonl', help="Path to save the retrieval results (JSONL format).")
    parser.add_argument('--output_path', type=str, default='/data/groups/QY_LLM_Other/meisen/dataset/rankcot/result_data/queryCoT_to_answer.jsonl', help="Path to save the generate results (JSONL format).")
    ...
    generator = COTRefineGenerator(args.input_path, args.output_path, args.model_name_or_path, args.config_path)
    generator.run()
